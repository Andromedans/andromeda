...
let rec equal_at ctx e1 e2 t =
  let t = Norm.whnf ctx t in
    match fst t with
      ...
      | Refl _ | Transport _ | TyWtn _ | EqWtn _ | Lambda _ | Subst _ | Ascribe _ ->
...
and equal ctx e1 e2 =
  ...
  let e2 = Norm.whnf ctx e2 in
    match fst e1, fst e2 with
      ...
      | (Var _ | Id _ | Refl _ | Transport _ | Pi _ | Lambda _ | App _ | Subst _ |
      ...
      (match equal_spine ctx a1 b1 with
        ...
        | Some t ->
          (match fst (Norm.whnf ctx t) with
            ...
            | _ -> None))
    | (Var _ | Id _ | Refl _ | Transport _ | Pi _ | Lambda _ | App _ |
...
and check ctx ((e', loc) as e) t =
  ...
  match e' with
    ...
    | Var _ | Lambda (_, Some _, _) | Id _ | Refl _ | Transport _ | Pi _ | App _ |
...
and check_sort ctx (e',loc) =
  match e' with
    ...
    | Id (e1, e2, t) ->
      ...
      check ctx e2 t ;
      Small
    | Refl _ -> Error.typing ~loc "this expression is a reflexivity term but should be a sort"
    | Transport _ -> Error.typing ~loc "this expression is a transport term but should be a sort"
    | Pi (x, t1, t2) ->
      let u1 = check_sort ctx t1 in
      let u2 = check_sort (add_parameter x t1 ctx) t2 in
        max_size u1 u2
