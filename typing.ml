...
and equal ctx e1 e2 =
  ...
  let e2 = Norm.whnf ctx e2 in
    match fst e1, fst e2 with
      ...
      | (Var _ | Pi _ | Lambda _ | App _ | Subst _ | Ascribe _ | Type | Kind | EqWtn | TyWtn | EqJdg _ | TyJdg _), _ -> false
      ...
      (match equal_spine ctx a1 b1 with
        ...
        | Some t ->
          (match fst (Norm.whnf ctx t) with
            ...
            | _ -> None))
    | (Var _ | Pi _ | Lambda _ | App _ | Subst _ | Ascribe _ | Type | Kind | EqWtn | TyWtn | EqJdg _ | TyJdg _), _ -> None
...
let rec infer ctx (e, loc) =
  match e with
    ...
    | TyWtn -> Error.typing ~loc "why am I inferring the type of a typing judgment?"

    | EqWtn -> Error.typing ~loc "why am I inferring the type of an equality judgment?"
...
and check ctx ((e', loc) as e) t =
  ...
  match e' with
    ...
    | Kind -> Error.typing ~loc "internal error, cannot check sort"
...
and infer_pi ctx e =
  let t = infer ctx e in
    match fst (Norm.whnf ctx t) with
      ...
      | Var _ | App _ | Type | Kind | EqJdg _ | TyJdg _ | TyWtn | EqWtn | Lambda _ ->
