...
and equal ctx e1 e2 =
  ...
  let e2 = Norm.whnf ctx e2 in
    match fst e1, fst e2 with
      ...
      | (Var _ | Type | Pi _ | Lambda _ | App _ | Subst _ | Ascribe _), _ -> false
      ...
      (match equal_spine ctx a1 b1 with
        ...
        | Some t ->
          (match fst (Norm.whnf ctx t) with
            ...
            | _ -> None))
    | (Var _ | Type | Pi _ | Lambda _ | App _ | Subst _ | Ascribe _), _ -> None
...
let rec infer ctx (e, loc) =
  match e with
    ...
    | Type -> Error.typing ~loc "Type is not allowed here"
    | Pi (x, t1, t2) ->
      check ctx t1 mk_type ;
      check ctx t2 mk_type ;
      mk_type
...
and check ctx ((e', loc) as e) t =
  ...
  match e' with
    ...
    | Var _ | Lambda (_, Some _, _) | Pi _ | App _ | Ascribe _ ->
