...
let normalize ctx = 
  ...
  let rec normalize env (e, loc) : expr =
    match e with
      | Var x ->
        begin
          try
      ...
      | App (e1, e2) ->
        let e2 = normalize env e2 in
          (match normalize env e1 with
            | Lambda (x, _, e1'), _ -> normalize ((x, Some e2) :: env) e1'
            | e1 -> (App (e1, e2), loc))
      | Universe k -> (Universe k, loc)
      | Pi a -> (Pi (normalize_abstraction env a), loc)
      | Lambda a -> (Lambda (normalize_abstraction env a), loc)
