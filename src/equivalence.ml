...
module type EQUIV_ARG = sig
  ...
  val lookup_definition : Common.debruijn -> env -> term option
  ...
  (*val as_pair : env -> term -> term * handled_result*)
...
*)
  ...
  *)
  and equal_whnfs env exp1 exp2 =
    ...
    let exp1' = whnf env exp1 in
    P.debug "exp1' = %t@." (X.print_term env exp1') ;

    let exp2' = whnf env exp2 in
      ...
      | None ->
          begin
            ...
            | _, S.Ind_eq(S.Ju, _, _, _, _, _, _) ->
                Error.impossible
                   "equal_whnfs: whnf didn't eliminate Ind_eq(Ju,...)"
              ...
              begin
                ...
                | None   ->
                  begin
                     ...
                     * message as a string rather than immediately printing it... *)
  ...
  (** [whnf env e] reduces expression [e] in environment [env] to
    ...
    match e with
        ...
        | S.Var k ->
            begin
              match X.lookup_definition k env with
              | Some e' -> whnf env e'
              | _       -> e
            end
        ...
        | S.Handle (e, es) -> whnf env e

        | S.MetavarApp mva ->
            begin
              match S.get_mva mva with
              | None -> e
              | Some defn -> whnf env defn
            end
  ...
  (** [nf env e] reduces expression [e] in environment [env] to a normal form *)
  let rec nf env e =
      ...
      match whnf env e with
        ...
        | S.Lambda (x, t1, t2, e1) ->
            let t1' = nf env t1  in
            let env' = X.add_parameter x t1' env  in
            let t2' = nf env' t2   in
            let e1' = nf env' e1  in
            S.Lambda (x, t1', t2', e1')
        ...
        | S.Pair (e1, e2, x, ty1, ty2) ->
            let e1' = nf env e1  in
            let e2' = nf env e2  in
            let ty1' = nf env ty1  in
            let ty2' = nf (X.add_parameter x ty1 env) ty2  in
            S.Pair(e1', e2', x, ty1', ty2')
        ...
        | S.Pi (x, t1, t2) ->
            let t1' = nf env t1  in
            let e1' = nf (X.add_parameter x t1' env) t2  in
            S.Pi (x, t1', e1')
        ...
        | S.Ind_eq(S.Pr, t, (x,y,p,c), (z,w), a, b, q) ->
            ...
            let ctx_c = X.add_parameter p (S.shift 2 (S.Eq(S.Pr, a, b, t)))
                             (X.add_parameter y (S.shift 1 t)
                                (X.add_parameter x t env))  in
            let ctx_w = X.add_parameter z t env in
            let t' = nf env t  in
            let c' = nf ctx_c c  in
            let w' = nf ctx_w w  in
            let a' = nf env a  in
