...
let rec type_of_term_abstraction sgn = function
  ...
  | Abstract (x, t, abstr) ->
     ...
     Abstract.abstraction Abstract.is_type a.atom_nonce t_abstr

let judgement_with_boundary sgn jdg =
  match jdg with
...
let rec abstracted_judgement_with_boundary sgn abstr_jdg =
  match abstr_jdg with
    | Abstract (x, t, abstr) ->
      ...
      Abstract.judgement_with_boundary_abstraction a abstr_pair

    | NotAbstract jdg ->
      Mk.not_abstract (judgement_with_boundary sgn jdg)

let rec derivation_with_boundary sgn der =
  match der with
