...
and require_equal_ty ~loc env ctx (Tt.Ty lte) (Tt.Ty rte) (f : Context.t -> 'a Value.result) error : 'a Value.result =
  require_equal ~loc env ctx lte rte (Tt.mk_type_ty ~loc) f error
