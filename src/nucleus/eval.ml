...
and check env ((c',loc) as c) (((ctx_check, t_check') as t_check) : Judgement.ty) : (Context.t * Tt.term) Value.result =
  ...
  | Syntax.Bracket _  ->
    ...
    infer env c >>= as_term ~loc >>=
      (fun (ctxe, e, t') ->
       ...
       if Equal.equal_ty env ctx t_check' t'
  ...
  | Syntax.Ascribe (c1, c2) ->
     check_ty env c2 >>=
       (fun ((ctxt, t') as t) ->
        ...
        if Equal.equal_ty env ctx t' t_check'
        then
          check env c1 t >>=
            (fun (ctx1, e1) ->
  ...
  | Syntax.Refl c ->
    ...
    check env c t >>=
      (fun (ctxe, e) ->
       let ctx, eqs = Context.join ctx ctxe in
       if Equal.equal env ctx e e1 t'
       then if Equal.equal env ctx e e2 t'
