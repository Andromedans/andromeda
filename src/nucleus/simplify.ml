...
let rec simplify env ((e',loc) as e) =
    match e' with
    ...
    | Tt.Spine (e, (xts, t), es) ->
      simplify_spine ~loc env e xts t es
    ...
    | Tt.Prod (xts, t) ->
      let rec fold env ys xts = function
        ...
        | (x,u) :: xus ->
          let u = Tt.unabstract_ty ys 0 u in
          let u = simplify_ty env u in
          let y, env = Env.add_fresh env ~loc x u
    ...
    | Tt.Eq (t, e1, e2) ->
      let t = simplify_ty env t
      and e1 = simplify env e1
      and e2 = simplify env e2 in
        Tt.mk_eq ~loc t e1 e2

    | Tt.Refl (t, e) ->
      let t = simplify_ty env t
      and e = simplify env e in
        Tt.mk_refl ~loc t e
...
and simplify_ty env (Tt.Ty e) =
  let e = simplify env e in
    Tt.ty e

and simplify_spine ~loc env h xts t es =
