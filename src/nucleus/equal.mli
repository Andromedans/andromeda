...
val equal : Env.t -> Tt.term -> Tt.term -> Tt.ty -> bool

(** [equal_ty env t1 t2] checks whether types [t1] and [t2] are equal. *)
val equal_ty : Env.t -> Tt.ty -> Tt.ty -> bool

(** [whnf env e] reduces expression [e], assuming that it has a type. *)
val whnf : Env.t -> Tt.term -> Tt.term

(** Convert a type to a product. *)
val as_prod : Env.t -> Tt.ty -> (Tt.ty, Tt.ty) Tt.abstraction option

(** Convert a type to a bracket type. *)
val as_bracket : Env.t -> Tt.ty -> Tt.ty option
...
val as_universal_eq : Env.t -> Tt.ty -> (Tt.ty, Tt.ty * Tt.term * Tt.term) Tt.abstraction
...
val as_universal_bracket : Env.t -> Tt.ty -> (Tt.ty, Tt.ty) Tt.abstraction

(** [inhabit_bracket env t] attempts to inhabit the bracket type [[t]] using
    inhabit hints. *)
val inhabit_bracket : subgoals:bool -> loc:Location.t -> Env.t -> Tt.ty -> Tt.term option
