...
val equal_ty : Environment.t -> Context.t -> Tt.ty -> Tt.ty -> Context.t Opt.opt

(** [whnf env ctx e] reduces expression [e], assuming that it has a type in context [ctx]. *)
val whnf : Environment.t -> Context.t -> Tt.term -> (Context.t * Tt.term) Monad.t

(** [whnf_ty env ctx t] reduces type [t], assuming that it is a type in context [ctx]. *)
val whnf_ty : Environment.t -> Context.t -> Tt.ty -> (Context.t * Tt.ty) Monad.t

(** Convert a term to an atom. *)
val as_atom : Environment.t -> Judgement.term -> (Context.t * Name.atom * Tt.ty) Monad.t

(** Convert a type to an equality type. *)
val as_eq : Environment.t -> Judgement.ty -> (Context.t * Tt.ty * Tt.term * Tt.term) Monad.t

(** Convert a type to a product. *)
val as_prod : Environment.t -> Judgement.ty -> (Context.t * Tt.ty Tt.ty_abstraction) Monad.t
...
val as_universal_eq :
  Environment.t -> Judgement.ty -> (Context.t * (Tt.ty * Tt.term * Tt.term) Tt.ty_abstraction) Monad.t
...
val as_universal_bracket :
  Environment.t -> Judgement.ty -> (Context.t * Tt.ty Tt.ty_abstraction) Monad.t
...
val inhabit_bracket :
  ...
  Environment.t -> Judgement.ty -> (Context.t * Tt.term) Opt.opt

(** Convert a type to a bracket type. *)
val as_bracket : Environment.t -> Judgement.ty -> (Context.t * Tt.ty) Monad.t

(** Convert a type to a signature. *)
val as_signature : Environment.t -> Judgement.ty -> (Context.t * Tt.signature) Monad.t
