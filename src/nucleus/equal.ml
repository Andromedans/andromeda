    ...
    begin match e' with
      ...
      | Tt.Spine (e, (xts, t), (_::_ as es)) ->
        begin
          ...
          | Tt.Lambda (xus, (e', u)) ->
            begin
              beta_reduce ~loc:eloc env ctx xus e' u xts t es >>= function
                | None -> Value.return (ctx, Tt.mention assumptions (Tt.mk_spine ~loc e xts t es))
          ...
          | Tt.Structure _
          | Tt.Projection _ ->
             Value.return (ctx, Tt.mention assumptions (Tt.mk_spine ~loc e xts t es))
          | Tt.Bound _ ->
      ...
      | Tt.Projection (e,xts,p) ->
        begin
          ...
          match e' with
            | Tt.Structure xtes ->
              begin
                projection_reduce ~loc:eloc env ctx xts p xtes >>= function
                  | None -> Value.return (ctx, Tt.mention assumptions (Tt.mk_projection ~loc e xts p))
                  | Some (ctx,e) -> weak ctx (Tt.mention assumptions e)
              end
            | Tt.Atom _
            | Tt.Constant _
            ...
            | Tt.Signature _
            | Tt.Projection _ ->
               Value.return (ctx, Tt.mention assumptions (Tt.mk_projection ~loc e xts p))
            | Tt.Bound _ ->
...
and equal_signature ~loc env ctx xts1 xts2 =
  let rec fold env ctx ys ts xts1 xts2 = match xts1, xts2 with
    | [], [] -> opt_return (Context.abstract ~loc ctx ys ts) (* do we need to abstract here? *)
...
and pattern_collect_whnf env ctx p ~at_ty ({Tt.term=e';loc;_} as e_orig) =
  ...
  | Pattern.Term (e',t') ->
    opt_return ([], [CheckEqualTy ([], (t', at_ty)); CheckEqual (e', e_orig, at_ty)])
...
and inhabit_whnf ~subgoals env ctx ((Tt.Ty {Tt.term=t';loc;_}) as t) =
  ...
  match t' with
    ...
    | Tt.Prod (xts', t') ->
      let rec fold env ys ts = function
        | [] ->
...
let rec snf env ctx t =
  whnf env ctx t >>= fun (ctx, ({Tt.term=t';loc;_} as t)) ->
  match t' with
    ...
    | Tt.Lambda (abs,(te,ty)) ->
      let rec fold env ctx ys ts abs = function
        | [] ->
        ...
        | (x,t)::rem ->
          ...
          fold env ctx (y::ys) (t::ts) ((x,tabs)::abs) rem
      in
      fold env ctx [] [] [] abs
    ...
    | Tt.Spine (lhs,(abs,out),rhs) ->
      ...
      let rec fold env ctx ys ts abs = function
        ...
        | (x,t)::rem ->
          ...
          let jx = Judgement.mk_ty ctx t in
          let y, env = Environment.add_fresh ~loc env x jx in
          let tabs = Tt.unabstract_ty ys t in
          fold env ctx (y::ys) (t::ts) ((x,tabs)::abs) rem
      in
      fold env ctx [] [] [] abs
    ...
    | Tt.Signature xts ->
      let rec fold env ctx ys ts xts = function
        ...
        | (l,x,t)::rem ->
          ...
          let y, env = Environment.add_fresh ~loc env x jx in
          let tabs = Tt.abstract_ty ys t in
          fold env ctx (y::ys) (t::ts) ((l,x,tabs)::xts) rem
      in
      fold env ctx [] [] [] xts

    | Tt.Structure xes ->
      let rec fold env ctx ys ts xes = function
        | [] ->
        ...
        | (l,x,t,e)::rem ->
          ...
          fold env ctx (y::ys) (t::ts) ((l,x,tabs,e)::xes) rem
      in
      fold env ctx [] [] [] xes
