...
type check =
  ...
  | CheckEqualTy of (Pattern.pty * Tt.ty, Pattern.pty * Tt.ty) Tt.abstraction (* compare types in environment *)
    ...
    begin match e' with
      ...
      | Tt.Spine (e, (xts, t), (_::_ as es)) ->
        begin
          ...
          | Tt.Lambda (xus, (e', u)) ->
            begin
              match beta_reduce ~loc:eloc env xus e' u xts t es with
...
and whnf env e =
    ...
    | ((xts, (p, e')) as h) :: hs ->
      ...
      begin try
        ...
        Print.debug
          ...
          (Tt.print_term xs e) ;
        begin match verify_match ~spawn:false env xts pvars checks with
        | None ->
  ...
  if !Config.ignore_hints then e
    ...
    let e = apply_beta (Env.beta_hints key env) in
...
and beta_reduce ~loc env xus e u yvs t es =
  ...
  in
    ...
    in
      if not (equal_abstracted_ty env xuvs u' t')
...
(** Let [xuus] be the list [(x1,(u1,u1')); ...; (xn,(un,un'))] where
    [ui]  is well-formed in the environment [x1:u1 , ..., x{i-1}:u{i-1} ] and
    [ui'] is well-formed in the environment [x1:u1', ..., x{i-1}:u{i-1}'] and
    [v]  is well-formed in the environment [x1:u1, ..., xn:un] and
    [v'] is well-formed in the environment [x1:u1',..., xn:un'].
    We verify that the [ui] are equal to [ui'] and that [v] is equal to [v]. *)
and equal_abstracted_ty env (xuus : (Name.ident * (Pattern.pty * Tt.ty)) list) v v' =
    ...
    function
     ...
     | (x,(u,u'))::xuus ->
        ...
        in
          equal_ty env u u'
          &&
          (let y, env = Env.add_fresh ~loc:Location.unknown env x u in
             eq (ys @ [y]) env xuus) (* XXX optimize list append *)
   in
     eq [] env xuus

(** Compare two types *)
and equal_ty env (Tt.Ty t1) (Tt.Ty t2) = equal env t1 t2 Tt.typ

and equal env ((_,loc1) as e1) ((_,loc2) as e2) t =
  let xs = Env.used_names env in
  let i = cnt () in
  ...
  alpha_equal e1 e2 ||
    begin (* type-directed phase *)
      let (Tt.Ty (t',_)) as t = whnf_ty env t in
      match t' with

        | Tt.Type ->
          equal_hints env e1 e2 t
              ...
              |  ((xts, (pt, k1, k2)) as h) :: hs ->
                ...
                begin match collect_for_eta env (pt, k1, k2) (t, e1, e2) with
                  ...
                  | Some (pvars, checks) ->
                    ...
                    begin match verify_match ~spawn:true env xts pvars checks with
        ...
        | Tt.Prod (xus, u) ->
            let rec fold env ys es =
              ...
              | (x, ((Tt.Ty (_, loc)) as v)) :: xvs ->
                  ...
                  let y, env =  Env.add_fresh ~loc env x v in
...
and equal_hints env e1 e2 t =
  ...
  (* XXX can break general hints on functions. First note that when we get
     ...
     apply anymore *)
  let (e1',loc1) as e1 = whnf env e1
  and (e2',loc2) as e2 = whnf env e2
  in
    ...
    begin
      ...
      List.exists
        (fun (xts, (pt, pe1, pe2) as h) ->
          ...
          match collect_for_hint env (pt, pe1, pe2) (t, e1, e2) with
            | None -> false
            | Some (pvars, checks) ->
              (* check validity of the match *)
        ...
        (if !Config.ignore_hints then [] else Env.general_hints key env)
    end
    ||
    (* proceed with comparing the weak head normal forms *)
    equal_whnf env e1 e2

(* Compare normalized expressions. The assumption is that they both
   have a common type. *)
and equal_whnf env (e1',loc1) (e2',loc2) =
    ...
    begin match e1', e2' with
      ...
      | Tt.Constant (x1, es1), Tt.Constant (x2, es2) ->
        ...
        begin
          let yts, _ =
            begin match Env.lookup_constant x1 env with
            | Some ytsu -> ytsu
      ...
      | Tt.Spine (e, (yus, v), es) ->
        ...
        | _::_ ->
          Print.debug "found unexpected trailing arguments at %t"
            (Tt.print_term (Env.used_names env) (e', loc));
          raise NoMatch
        | [] ->
...
*)
and verify_match ~spawn env xts pvars checks =
      ...
      (function
        | CheckEqual (e1, e2, t) ->
          ...
          Print.debug "CheckEqual at@ %t@ %t@ %t"
            (Tt.print_ty (Env.used_names env) t)
            (Tt.print_term (Env.used_names env) e1)
            (Tt.print_term (Env.used_names env) e2);
  ...
  match t' with
    ...
    | Tt.Prod (xts', t') ->
      let rec fold env ys = function
        ...
        | (x,t)::xts ->
          let t = Tt.unabstract_ty ys 0 t in
          let y, env = Env.add_fresh ~loc env x t in
            fold env (y :: ys) xts
      in
        fold env [] xts'
...
and inhabit_bracket ~subgoals ~loc env t =
  if
    not subgoals ||
    begin
      (* apply inhabit hints *)
      ...
      List.exists
        ...
        (Env.inhabit_hints key env)
    end
  then Some (Tt.mk_inhab ~loc)
  else None

let rec as_universal_eq env t =
  let rec fold env xus ys t =
    let (Tt.Ty (t', loc)) = whnf_ty env t in
    match t' with
    ...
    | Tt.Prod ((_ :: _) as zvs, w) ->
      let rec unabstract_binding env zs' zvs w =
      begin
        match zvs with
        | [] ->
        ...
        | (z,v) :: zvs ->
          ...
          let z', env = Env.add_fresh ~loc env z v in
            unabstract_binding env (z' :: zs') zvs w
      end
      in
        let env, zs', w = unabstract_binding env [] zvs w in
          fold env (xus @ zvs) (zs' @ ys) w
...
let rec as_universal_bracket env t =
  let rec fold env xus ys t =
    let (Tt.Ty (t', loc)) as t = whnf_ty env t in
    match t' with
