...
let invert_eq_term sgn (EqTerm (asmp, e1, e2, t)) =
  ...
  let e1 = if Alpha_equal.is_type t1 t then e1 else Form_convert.is_term_convert sgn e1 (Mk.eq_type asmp t1 t)
  and e2 = if Alpha_equal.is_type t2 t then e2 else Form_convert.is_term_convert sgn e2 (Mk.eq_type asmp t2 t)
  in
