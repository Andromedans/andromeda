...
(** Extend the context with a series of variables with the given
    names and types.
...
(** Look up the given name in the context. If found, return the type (indexed
    relative to the given context) of the newest variable of that name. If no
    such variable exists, throws an exception. *)
...
(** [append ctx1 ctx2] returns a context containing all the entries in [ctx1]
    followed by the entries in [ctx2]. Note that it is assumed that [ctx2] is
    already well-formed (shifted) with respect to [ctx1]; this function does no
    further shifts of de Bruijn indices. *)
val append : t -> t -> t
