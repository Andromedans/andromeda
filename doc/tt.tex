...
\begin{mathpar}
  ...
  \infer[eval-makeapp]
  {
    ...
    \mathsf{typeOf}(B_2) = \U_1\\
    \\\\
    \evalto
      {\bigl(\letin{\X = \inhabitOp{(\T_1\equiv\U_1)}}
         {\val (\equationin{\X}{\T_1}{\U_1}{(\brazilapp{B_1}{\x}{\T_1}{\T_2}{B_2})})}\bigr)}
      {R}
  }
  { \evalto
      {\makeApp{B_1}{B_2}}
...
This last rule isn't quite right, because $T_1$ and $U_1$ are types, and so
$\T_1\equiv\U_1$ is not a Brazil type.

Also, either here or (more likely) in Rule~\textsc{handle-op}, we need to check that the result of an inhabit operation (i.e., the value passed to the continuation) is a Brazil term of the correct type. That also might invoke handlers\ldots
