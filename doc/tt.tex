...
\newcommand{\ctx}{\Gamma}
\newcommand{\ctxextend}[3][\ctx]{#1, #2 {:} #3}

\newcommand{\evalto}[3][\ctx]{#1 \vdash #2 \Downarrow #3}
\newcommand{\resultok}[2][\ctx]{#1 \vdash #2 \ \mathsf{ok}}
\begin{document}
...
%
\begin{align*}
  &\evalto[\ctx]{C}{R} &&\text{$C$ evaluates to $R$ in context $\ctx$} \\
  &\resultok[\ctx]{R}  &&\text{$R$ is a valid result in context $\ctx$}
\end{align*}
%
Rules:
\begin{mathpar}
