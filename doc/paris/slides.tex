...
\renewcommand{\rulename}[1]{\text{\textsc{\small #1}}}
...
\begin{frame}
  ...
  Application (synthetis \& checking):
  %
  \begin{equation*}
  ...
  \note[item]{
    For example, consider the traditional appication rule and how it is done
...
\begin{frame}
  ...
  \note[item]{Our version consults the hints. For example, to check that $e_1$ and $e_2$
    ...
    consult the hints, then we proceed with the Harper-Stone algoithm (which calls itself
