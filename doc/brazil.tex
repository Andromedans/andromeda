...
\newcommand{\cc}{c} % a term computation
\newcommand{\CC}{C} % a type computation
...
% computations
\newcommand{\handle}[2]{\mathsf{handle}\;#1\;\mathsf{with}\;#2} % handler
\newcommand{\val}[1]{\mathsf{val}\;#1} % pure value
\newcommand{\cheq}[3]{\mathsf{check}\;#1 \equiv #2 : #3\;\mathsf{in}\;}
\newcommand{\cheqty}[2]{\mathsf{check}\;#1 \equiv #2 \;\mathsf{in}\;}
\newcommand{\letin}[1]{\mathsf{let}\;#1\;\mathsf{in}\;} % let statement
\newcommand{\letty}[1]{\mathsf{type}\;#1\;\mathsf{in}\;} % let-type statement
...
\newcommand{\comptype}[3]{#1 \vdash #2 \mapsto #3 \;\mathsf{type}} % well formed type
\newcommand{\compfib}[3]{#1 \vdash #2 \mapsto #3  \;\mathsf{fibered}} % is a fibered type


\newcommand{\eqtype}[3]{#1 \vdash #2 \equiv #3} % equal types
\newcommand{\eqterm}[4]{#1 \vdash #2 \equiv #3 : #4} % equal terms

\newcommand{\chkterm}[4]{#1 \vdash #2 \mapsto #3 \Leftarrow #4} % well formed term
\newcommand{\synterm}[4]{#1 \vdash #2 \mapsto #3 \Rightarrow #4} % well formed term
...
\section{Bidirectional type theory with handlers}
\label{sec:bidir-type-theory}

We now define another theory which should capture the same type theory as the orthodox
one, but is amenable to algorithmic treatment.
...
\begin{equation*}
  \CC
  \begin{aligned}[t]
    \bnf   {}& \val{\T}                  &&\text{pure type}\\
    \bnfor {}& \cheq{\e_1}{\e_2}{\T} \CC &&\text{term equality check}\\
    \bnfor {}& \cheqty{\T}{\U} \CC       &&\text{type equality check}\\
    \bnfor {}& \letin{\x = c} \CC        &&\text{term binding}\\
    \bnfor {}& \letty{\T = \CC_1} \CC_2  &&\text{type binding}\\
    \bnfor {}& \handle{\CC}{\e}          &&\text{handling construct}
  \end{aligned}
\end{equation*}
%
Types:
...
\begin{equation*}
  ...
  \begin{aligned}[t]
    ...
    \bnfor {}& \cheq{\e_1}{\e_2}{\T} \cc &&\text{term equality check}\\
    \bnfor {}& \cheqty{\T}{\U} \cc       &&\text{type equality check}\\
    \bnfor {}& \letin{\x = \cc_1} \cc_2  &&\text{term binding}\\
    \bnfor {}& \letty{\x = \CC} \cc      &&\text{type  binding}\\
    \bnfor {}& \handle{\cc}{\e}          &&\text{handling construct}
  \end{aligned}
\end{equation*}
%
...
\begin{equation*}
  ...
  \begin{aligned}[t]
    ...
    \bnfor {}&  \lam{\x}{\T} \cc  &&\text{$\lambda$-abstraction} \\
    \bnfor {}&   \app{\e_1}{\e_2}  &&\text{application} \\
    \bnfor {}&  \unitTerm  &&\text{the element of unit type} \\
    \bnfor {}&  \prRefl{\T}{\e}  &&\text{identity path} \\
    \bnfor {}&  \PrElim{\T}{\abst{x\,y\,p}{\CC}}{\abst{z}{\cc}}{\e_1}{\e_2}{\e_3}  &&\text{path eliminator} \\
    \bnfor {}&  \juRefl{\T}{\e}  &&\text{reflexivity} \\
    \bnfor {}&  \JuElim{\T}{\abst{x\,y\,p}{\CC}}{\abst{z}{\cc}}{\e_1}{\e_2}{\e_3}  &&\text{equality eliminator} \\
    \bnfor {}&  \nUnit  &&\text{the name of unit type} \\
    \bnfor {}&  \nProd{\alpha,\beta}{\x}{\e} \cc  &&\text{the name of product type} \\
    \bnfor {}&  \nUniverse{\alpha} &&\text{the name of a universe} \\
    \bnfor {}&  \coerce{\e}{\alpha}{\beta}  &&\text{universe coercion} \\
    \bnfor {}&  \nPrEqual{\alpha}{\e_1}{\e_2}{\e_3}  &&\text{the name of a path type} \\
    \bnfor {}&  \nJuEqual{\alpha}{\e_1}{\e_2}{\e_3}  &&\text{the name of an equality type}
...
\begin{align*}
& \isctx{\G} & & \text{$\G$ is a well formed context} \\
& \comptype{\G}{\CC}{\T} & & \text{$\CC$ computes type $\T$ in context $\G$} \\
& \compfib{\G}{\CC}{\T} & & \text{$\CC$ computes fibered type $\T$ in context $\G$} \\
& \chkterm{\G}{\cc}{\e}{\T} & & \text{$\cc$ computes term $\e$ checked to have type $T$ in $\G$} \\
& \synterm{\G}{\cc}{\e}{\T} & & \text{$\cc$ computes term $\e$ and synthesizes its type $T$ in $\G$} \\
& \eqtype{\G}{\T}{\U} & & \text{$\T$ and $\U$ are equal types in context $\G$} \\
& \eqterm{\G}{\e_1}{\e_2}{\T} & & \text{$e_1$ and $e_2$ are equal terms of type $\T$ in context $\G$}
\end{align*}
...
%
\begin{mathpar}
  \infer[\rulename{compty-let-term}]
  {\synterm{\G}{\cc}{\e}{\U} \\
   \comptype{\G}{\subst{\CC}{x}{\e}}{\T}
  }
  {\comptype{\G}{(\letin{x = \cc} \CC)}{\T}}

  \infer[\rulename{compty-let-ty}]
  {\comptype{\G}{\CC_1}{\U} \\
   \comptype{\G}{\subst{\CC_2}{x}{\U}}{\T}
  }
  {\comptype{\G}{(\letty{x = \CC_1} \CC_2)}{\T}}

  \infer[\rulename{compty-handle}]
  {\isterm{\G}{\e}{\JuEqual{\U}{\e_1}{\e_2}} \\
   \comptype{\G}{\handle{\CC}{\e}}{\T}
  }
  {\comptype{\G}{(\handle{(\cheq{\e_1}{\e_2}{\U} \CC)}{\e})}{\T} }

  \infer[\rulename{comp-XXX}]
  { }
  { }
...
\begin{mathpar}
  \infer[\rulename{comp-universe}]
  {\isctx{\G} \\
   \alpha \in \PP
  }
  {\comptype{\G}{\val{\Universe{\alpha}}}{\Universe{\alpha}}}
