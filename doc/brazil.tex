...
\begin{mathpar}
  ...
  {\eqtype{\G}{\El{(\coerce{\alpha}{\beta}{\e})}}
              {\El{\e}}}
...
\begin{mathpar}
\infer[\rulename{eq-beta-alternate}]
  {\eqtype{\G}{\Prod{\x}{\T_1}{\T_2}}{\Prod{\x}{\U_1}{\U_2}}\\
    \isterm{\ctxextend{\G}{\x}{\U_1}}{\e_1}{\U_2}\\
    \isterm{\G}{\e_2}{\U_1}}
  {\eqterm{\G}{\bigl(\app{(\lam{\x}{\T_1}{\T_2}{\e_1})}{\x}{\U_1}{\U_2}{\e_2}\bigr)}
              {\subst{\e_1}{\x}{\e_2}}
              {\subst{\U_2}{\x}{\e_2}}}
...
This alternate formulation of the beta rule is more general. For example,
we can use it to derive:
\[
  \eqterm{w:\JuEqual{\Universe{\zero}}{\mathsf{Nat}{\to}\Unit}{\mathsf{Bool}{\to}\Unit}}
         {\bigl(\app{(\lam{\x}{\mathsf{Nat}}{\Unit}{\unitTerm})}
               {\_}{\mathsf{Bool}}{\Unit}
               {\mathsf{true}}\bigr)}
         {\unitTerm}
         {\Unit}
       \]
The cost is is that it's more
expensive to check whether this rule safe to apply during normalization.
In Rule~\rulename{eq-beta}, the well-formedness premises must hold
(if the beta-redex is well-formed) and don't need to be checked.
For Rule~\rulename{eq-beta-alternate}, we are guaranteed only that
$\isterm{\ctxextend{\G}{\x}{\T_1}}{\e_1}{\T_2}$ and so the well-formedness
premise
$\isterm{\ctxextend{\G}{\x}{\U_1}}{\e_1}{\U_2}$ would have to be
re-checked algorithmically at run-time.

Of course, Rule~\rulename{eq-beta-alternate} implies Rule~\rulename{eq-beta}, and the latter is by
far the most common case (i.e., the Pi types are equivalent because their domains are
equivalent and their codomains are equivalent, not because of some weird equation). So
in practice, we might always try to justify beta-reductions by \rulename{eq-beta},
and try the more expensive \rulename{eq-beta-alternate} only when that fails.
}
...
\begin{mathpar}
  ...
  { \synterm{\GH}{(\lam{\x}{\T}{\U} \e)}{\Prod{\x}{\T} \U} }

  \infer[\rulename{syn-app}]
  { \chkterm{\GH}{\e_1}{\Prod{\x}{\T} \U} \\
    \chkterm{\GH}{\e_2}{\T}
  }
  { \synterm{\GH}{\app{\e_1}{\x}{\T}{\U}{\e_2}}{\subst{\U}{\x}{\e_2}} }
\end{mathpar}
...
\begin{mathpar}
  ...
  \infer[\rulename{syn-j}]
  {
    ...
    \chkterm{\GH}{\e_3}{\T}\\
    \chkterm{\GH}{\e_4}{\T}\\
    \chkterm{\GH}{\e_2}{\PrEqual{\T}{\e_3}{\e_4}} \\
  }
  { \synterm{\GH}{
     \PrElim{\T}
             {\abst{x\,y\,p}{\U}}
...
{\color{blue}
  ...
  If we have Rule~\rulename{eq-beta-alternate}, then the algorithm can have this rule:

  \begin{mathpar}
  \infer[\rulename{norm-app-beta}]
  { \eqtypealg{\GH}{\Prod{\x}{\T_1}{\T_2}}{\Prod{\x}{\U_1}{\U_2}}\\
    \chkterm{\ctxs{(\ctxextend{\G}{\x}{\U_1})}{\H}}{\e_1}{\U_2}
  }
  { \whnf
    {\GH}
    {\app{(\lam{\x}{\T_1}{\T_2}{\e_1})}{\x}{\U_1}{\U_2}{\e_2}}
...
{\color{blue}
  Do we need further checks to ensure Rule~\rulename{norm-idpath} is valid and type-preserving?
}
...
\begin{mathpar}
  ...
  {
    \whnf{\GH}{(\coerce{\alpha}{\alpha}{\e})}{\e}
  }
\end{mathpar}
...
\begin{mathpar}
 ...
 {\whnf{\GH}{\e_1}{\e'_1}}
 {\whnf{\GH}{\app{\e_1}{\x}{\T}{\U}{\e_2}}{\app{\e'_1}{\x}{\T}{\U}{\e_2}}}
...
{\color{blue}
  The following \rulename{chk-eq-whnf-XXX} rules don't work, because we lost all the universe subscripts,
  and we're recursively invoking checking rules. Either we have to switch back
  to a synthesizing judgment (which would, among other things, require equivalence
  hints to be tagged with their types, or reconstruct that type from the hint terms
  on the fly), or put the subscripts back.
}
...
\begin{mathpar}
  ...
  {\alpha = \alpha' \\
   ...
   \eqpath{\ctxs{(\ctxextend{\G}{\x}{\El{\alpha}{\e_1}})}{\H}}{\e_2}{\e'_2}{\Universe{\beta}} \\
  }
  {\eqpath{\GH}
    {(\nProd{\alpha}{\beta}{\x}{\e_1}{\e_2})}
  ...
  \infer[\rulename{chk-eq-whnf-paths}]
  {
    ...
    \eqtermalg{\GH}{\e_1}{\e'_1}{\El{\alpha}{\e_{\T}}}\\
    \eqtermalg{\GH}{\e_2}{\e'_2}{\El{\alpha}{\e_{\T}}}
  }
  {\eqpath{\GH}{\nPrEqual{\alpha}{\e_T}{\e_1}{\e_2}}{\nPrEqual{\alpha'}{\e_U}{\e'_1}{\e'_2}}{\Universe{\alpha}}}
  ...
  {
    \alpha = \alpha'\\
    \eqtermalg{\GH}{\e_T}{\e_U}{\Universe{\alpha}}\\
    \eqtermalg{\GH}{\e_1}{\e'_1}{\El{\alpha}{\e_{\T}}}\\
    \eqtermalg{\GH}{\e_2}{\e'_2}{\El{\alpha}{\e_{\T}}}
  }
  {\eqpath{\GH}{\nJuEqual{\alpha}{\e_T}{\e_1}{\e_2}}{\nJuEqual{\alpha'}{\e_U}{\e'_1}{\e'_2}}{\Universe{\alpha}}}
  ...
  {
    \alpha = \beta \\
    \eqpath{\GH}{\e_1}{\e_2}{\Universe{\gamma}} \\
  }
  {
    \eqpath{\GH}{\coerce{\alpha}{\e_1}}
                {\coerce{\beta}{\e_2}}
                {\Universe{\beta}}
  }
